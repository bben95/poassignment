    ...
    <div className='absolute top-16 right-2 h-[60%] w-[28%] bg-white rounded-3xl drop-shadow-2xl flex flex-col items-center'>
      ...
      <div className="h-[28%] bg-white   px-8 overflow-y-scroll scrollbar-none ">
          {data?.map((item)=>(
            ...
            <div key={item.id} className="border-b-2 border-neutral-200 flex p-2 cursor-pointer items-center">
              <img src={item.profilepicture} className="w-8 h-8 rounded-full" alt={item.name} />
              <p className="text-sm font-light text-neutral-600 pl-4">{item.name}</p>
            </div>
        
          ))}
        </div>
         <button className='text-lg bg-red-700 text-white my-3 px-3 py-1 rounded-full' onClick={()=>{navigate('/')}}>Sign Out</button>
    </div>
